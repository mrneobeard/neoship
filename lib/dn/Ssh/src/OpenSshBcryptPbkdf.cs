...
internal static class OpenSshBcryptPbkdf
{
    private const int BlockSize = 32;
    private static readonly byte[] Magic = Encoding.ASCII.GetBytes("OxychromaticBlowfishSwatDynamite");

    public static byte[] Key(byte[] password, byte[] salt, int rounds, int keyLength)
    {
        ArgumentNullException.ThrowIfNull(password);
        ArgumentNullException.ThrowIfNull(salt);
        if (rounds < 1)
            throw new CryptographicException("OpenSSH bcrypt KDF rounds must be positive.");

        if (password.Length == 0)
            throw new CryptographicException("OpenSSH bcrypt KDF password must not be empty.");

        if (salt.Length == 0 || salt.Length > 1 << 20)
            throw new CryptographicException("OpenSSH bcrypt KDF salt length is invalid.");

        if (keyLength <= 0 || keyLength > 1024)
            throw new CryptographicException("OpenSSH bcrypt KDF key length is invalid.");

        var blocks = (keyLength + BlockSize - 1) / BlockSize;
        var key = new byte[blocks * BlockSize];
        var shaPass = SHA512.HashData(password);
        var countSalt = new byte[salt.Length + 4];
        salt.CopyTo(countSalt, 0);
        var tmp = new byte[BlockSize];

        for (var block = 1; block <= blocks; block++)
        {
            BinaryPrimitives.WriteUInt32BigEndian(countSalt.AsSpan(salt.Length, 4), checked((uint)block));
            BCryptHash(tmp, shaPass, SHA512.HashData(countSalt));
            var output = tmp.ToArray();
            for (var i = 2; i <= rounds; i++)
            {
                BCryptHash(tmp, shaPass, SHA512.HashData(tmp));
                for (var j = 0; j < output.Length; j++)
                    output[j] ^= tmp[j];
            }

            for (var i = 0; i < output.Length; i++)
                key[(i * blocks) + (block - 1)] = output[i];
        }

        return key[..keyLength];
    }

    private static void BCryptHash(byte[] output, byte[] shaPass, byte[] shaSalt)
    {
        var state = new BlowfishState();
        state.ExpandKeyWithSalt(shaPass, shaSalt);
        for (var i = 0; i < 64; i++)
        {
            state.ExpandKey(shaSalt);
            state.ExpandKey(shaPass);
        }

        Magic.CopyTo(output, 0);
        for (var offset = 0; offset < output.Length; offset += 8)
        {
            for (var i = 0; i < 64; i++)
                state.Encrypt(output, offset);
        }

        for (var i = 0; i < output.Length; i += 4)
            (output[i], output[i + 1], output[i + 2], output[i + 3]) = (output[i + 3], output[i + 2], output[i + 1], output[i]);
    }

    private sealed class BlowfishState
    {
        private readonly uint[] p = InitialP.ToArray();
        private readonly uint[] s0 = InitialS0.ToArray();
        private readonly uint[] s1 = InitialS1.ToArray();
        private readonly uint[] s2 = InitialS2.ToArray();
        private readonly uint[] s3 = InitialS3.ToArray();

        public void ExpandKey(byte[] key)
        {
            var pos = 0;
            for (var i = 0; i < this.p.Length; i++)
                this.p[i] ^= NextWord(key, ref pos);

            uint left = 0;
            uint right = 0;
            for (var i = 0; i < this.p.Length; i += 2)
            {
            ...
            this.ExpandSBox(this.s0, ref left, ref right);
            this.ExpandSBox(this.s1, ref left, ref right);
            this.ExpandSBox(this.s2, ref left, ref right);
            this.ExpandSBox(this.s3, ref left, ref right);
        }

        public void ExpandKeyWithSalt(byte[] key, byte[] salt)
        {
            var pos = 0;
            for (var i = 0; i < this.p.Length; i++)
                this.p[i] ^= NextWord(key, ref pos);
            ...
            for (var i = 0; i < this.p.Length; i += 2)
            {
                left ^= NextWord(salt, ref pos);
                right ^= NextWord(salt, ref pos);
                (left, right) = this.EncryptBlock(left, right);
                this.p[i] = left;
                this.p[i + 1] = right;
            }

            this.ExpandSBoxWithSalt(this.s0, salt, ref pos, ref left, ref right);
            this.ExpandSBoxWithSalt(this.s1, salt, ref pos, ref left, ref right);
            this.ExpandSBoxWithSalt(this.s2, salt, ref pos, ref left, ref right);
            this.ExpandSBoxWithSalt(this.s3, salt, ref pos, ref left, ref right);
        }

        public void Encrypt(byte[] data, int offset)
        {
            var left = BinaryPrimitives.ReadUInt32BigEndian(data.AsSpan(offset, 4));
            var right = BinaryPrimitives.ReadUInt32BigEndian(data.AsSpan(offset + 4, 4));
            (left, right) = this.EncryptBlock(left, right);
            BinaryPrimitives.WriteUInt32BigEndian(data.AsSpan(offset, 4), left);
            BinaryPrimitives.WriteUInt32BigEndian(data.AsSpan(offset + 4, 4), right);
        }

        private static uint NextWord(byte[] bytes, ref int position)
        {
            uint value = 0;
            for (var i = 0; i < 4; i++)
            {
        ...
        private void ExpandSBox(uint[] sBox, ref uint left, ref uint right)
        {
        ...
        private void ExpandSBoxWithSalt(uint[] sBox, byte[] salt, ref int position, ref uint left, ref uint right)
        {
            for (var i = 0; i < sBox.Length; i += 2)
            {
                left ^= NextWord(salt, ref position);
                right ^= NextWord(salt, ref position);
                (left, right) = this.EncryptBlock(left, right);
                sBox[i] = left;
                sBox[i + 1] = right;
            }
        ...
        private (uint Left, uint Right) EncryptBlock(uint left, uint right)
        {
            unchecked
            {
                left ^= this.p[0];
                right ^= this.F(left) ^ this.p[1];
                left ^= this.F(right) ^ this.p[2];
                right ^= this.F(left) ^ this.p[3];
                left ^= this.F(right) ^ this.p[4];
                right ^= this.F(left) ^ this.p[5];
                left ^= this.F(right) ^ this.p[6];
                right ^= this.F(left) ^ this.p[7];
                left ^= this.F(right) ^ this.p[8];
                right ^= this.F(left) ^ this.p[9];
                left ^= this.F(right) ^ this.p[10];
                right ^= this.F(left) ^ this.p[11];
                left ^= this.F(right) ^ this.p[12];
                right ^= this.F(left) ^ this.p[13];
                left ^= this.F(right) ^ this.p[14];
                right ^= this.F(left) ^ this.p[15];
                left ^= this.F(right) ^ this.p[16];
                right ^= this.p[17];
                return (right, left);
            }
        }
        ...
        private uint F(uint value)
        {
            unchecked
            {
                return ((this.s0[(byte)(value >> 24)] + this.s1[(byte)(value >> 16)]) ^ this.s2[(byte)(value >> 8)]) + this.s3[(byte)value];
            }
        }
    }
    ...
    private static readonly uint[] InitialS0 =
    [
    ...
    private static readonly uint[] InitialS1 =
    [
    ...
    private static readonly uint[] InitialS2 =
    [
    ...
    private static readonly uint[] InitialS3 =
    [
