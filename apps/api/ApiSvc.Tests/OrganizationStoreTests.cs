...
public class OrganizationStoreTests
{
    ...
    public async Task CreateAsync_NormalizesSlugAndAssignsUser()
    {
        ...
        Assert.True(await db.RoleAssignments.AnyAsync(x => x.OrgId == org.Id && x.UserId == user.Id && x.RoleKey == BuiltInRoleStore.OwnerRoleName, TestContext.Current.CancellationToken));
    }
